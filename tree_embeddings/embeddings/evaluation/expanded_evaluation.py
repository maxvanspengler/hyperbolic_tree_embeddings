...
from hypll.manifolds.poincare_ball.math.diffgeom import dist
from hypll.tensors import ManifoldTensor


# Increase to reduce run time, decrease to decrease memory usage
CHUNK_SIZE = 2048


def dist(x: FPETensor, y: FPETensor, c: float | torch.Tensor) -> torch.Tensor:
...
def distortion(
    embeddings: FPETensor,
    graph: nx.DiGraph | nx.Graph,
    ball: PoincareBall,
    tau: float,
) -> FPETensor:
    # Set some stuff up for computing target distances later
    undirected_graph = graph.to_undirected()
    number_of_nodes = embeddings.size(0)

    # Compute pairwise distances of embeddings (in chunks because memory problems)
    embedding_dists = FPETensor.from_tensor(
        torch.zeros(number_of_nodes, number_of_nodes, embeddings.terms, dtype=embeddings.dtype)
    )
    chunks = math.ceil(number_of_nodes / CHUNK_SIZE)
    for i in range(chunks):
        for j in range(chunks):
            chunk_dists = dist(
                embeddings[i * CHUNK_SIZE:(i + 1) * CHUNK_SIZE, None, :],
                embeddings[None, j * CHUNK_SIZE:(j + 1) * CHUNK_SIZE, :],
                ball.c(),
            )
            embedding_dists[
    ...
    embedding_dists = dist(embeddings[None, :, :], embeddings[:, None, :], ball.c())
...
def mean_average_precision(
    ...
    embedding_dists = dist(embeddings[None, :, :], embeddings[:, None, :], ball.c()).tensor[..., 0]
